...
ShogunComponent::ShogunComponent(ComponentId_t id, Params &params)
    ...
    output->verbose(CALL_INFO, 1, 0, "Creating Shogun crossbar at %s clock rate and %"
    PRIi32
    " ports\n",
    ...
    if (port_count <= 0) {
        output->fatal(CALL_INFO, -1,
                      "Error: you specified a port count of less than or equal to zero.\n");
    }

    output->verbose(CALL_INFO, 1, 0, "Connecting %"
    PRIi32
    " links...\n", port_count);
    ...
    for (int32_t i = 0; i < port_count; ++i) {
        sprintf(linkName, "port%"
        PRIi32, i);
        output->verbose(CALL_INFO, 1, 0, "Configuring port %s ...\n", linkName);

        links[i] = configureLink(linkName, new Event::Handler<ShogunComponent>(this,
                                                                               &ShogunComponent::handleIncoming));

        if (nullptr == links[i]) {
            output->fatal(CALL_INFO, -1, "Failed to configure link on port %"
            PRIi32
            "\n", i);
        }
    }
    ...
    inputQueues = (ShogunQueue<ShogunEvent *> **) malloc(
        sizeof(ShogunQueue<ShogunEvent *> *) * port_count);
...
bool ShogunComponent::tick(SST::Cycle_t currentCycle) {
    output->verbose(CALL_INFO, 4, 0, "TICK() START [%30"
    PRIu64
    ...
    arb->moveEvents(input_message_slots, port_count, inputQueues, output_message_slots,
    ...
    output->verbose(CALL_INFO, 4, 0, "Pending event count: %"
    PRIi32
    "\n", pending_events);
...
void ShogunComponent::init(unsigned int phase) {
    ...
    if (0 == phase) {
        for (int32_t i = 0; i < port_count; ++i) {
            links[i]->sendUntimedData(
                new ShogunInitEvent(port_count, i, inputQueues[i]->capacity()));
        }
    }
            ...
            if (nullptr != ev) {
                ...
                if ((nullptr == initEv) && (nullptr == creditEv)) {
                    ...
                    for (int32_t j = 0; j < port_count; ++j) {
                        if (i != j) {
                            output->verbose(CALL_INFO, 4, 0, "sending untimed data from %"
                            PRIi32
                            " to %"
                            PRIi32
                            "\n", i, j);
...
void ShogunComponent::emitOutputs() {
    ...
    for (int32_t i = 0; i < port_count; ++i) {
        output->verbose(CALL_INFO, 4, 0, "-> Processing port %"
        PRIi32
        ":\n", i);

        for (uint32_t j = 0; j < output_message_slots; ++j) {
            if (nullptr != pendingOutputs[i][j]) {
                output->verbose(CALL_INFO, 4, 0, "  -> output is not null, remote-slot-count: %"
                PRIi32
                ", src=%5"
                PRIi32
                ...
                if (remote_output_slots[i] > 0) {
                    output->verbose(CALL_INFO, 4, 0, "    -> sending event (has entry and free %"
                    PRIi32
                    " slots)\n", remote_output_slots[i]);
                    ...
                    output->verbose(CALL_INFO, 4, 0,
                                    "    -> no free slots, event send disabled for this round (slots: %"
                    PRIi32
                    ")\n", remote_output_slots[i]);
                }
            }
...
void ShogunComponent::printStatus() {
    output->verbose(CALL_INFO, 4, 0,
                    "BEGIN: processing x-bar inputs -----------------------------------------------\n");
    output->verbose(CALL_INFO, 4, 0,
                    "BEGIN X-BAR STATUS REPORT ====================================================\n");

    for (int32_t i = 0; i < port_count; ++i) {
        output->verbose(CALL_INFO, 4, 0, "port %5"
        PRIi32
        " / in-q-count: %5"
        PRIi32
        " / remote-slots: %5"
...
void ShogunComponent::handleIncoming(SST::Event *event) {
    ...
    if (nullptr != incomingShogunEv) {
        ...
        if (inputQueues[src_port]->full()) {
            output->fatal(CALL_INFO, 4, 0, "Error: recv event for port %"
            PRIi32
            " but queues are full\n", src_port);
        }

        output->verbose(CALL_INFO, 4, 0, "-> recv from %"
        PRIi32
        " dest: %"
        PRId64
        "\n",
        ...
        if (nullptr != creditEv) {
            ...
            output->verbose(CALL_INFO, 4, 0, "-> recv-credit from %"
            PRIi32
            "\n", src_port);
