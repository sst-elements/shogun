...
ShogunNIC::ShogunNIC(SST::Component *component, Params &params)
    : SimpleNetwork(component), netID(-1), bw(UnitAlgebra("1GB/s")) {
...
SimpleNetwork::Request *ShogunNIC::recvInitData() {
    output->verbose(CALL_INFO, 8, 0, "Recv init-data on net: %5"
    PRId64
    " init-events have %5zu events.\n", netID, initReqs.size());
...
bool ShogunNIC::send(SimpleNetwork::Request *req, int vn) {
    if (netID > -1) {
        output->verbose(CALL_INFO, 8, 0, "Send: remote-slots: %5"
        PRIi32
        "\n", remote_input_slots);
        ...
        if (remote_input_slots > 0) {
            ...
            remote_input_slots--;
            output->verbose(CALL_INFO, 8, 0, "-> sent, remote slots now %5"
            PRId32
            ", dest=%5"
            PRId64
            ...
            output->verbose(CALL_INFO, 8, 0,
                            "-> called send but no free remote slots, so cannot send request\n");
            return false;
        }
    } else {
        output->fatal(CALL_INFO, -1,
                      "Error: send operation was called but the network is not configured yet (netID still equals -1)\n");
    }
...
SimpleNetwork::Request *ShogunNIC::recv(int vn) {
    if (netID > -1) {
        output->verbose(CALL_INFO, 8, 0, "Recv called, pending local entries: %"
        PRIi32
        "\n", reqQ->count());
        ...
        if (!reqQ->empty()) {
            ...
            if (nullptr != req) {
                output->verbose(CALL_INFO, 8, 0, "-> request src: %"
                PRId64
                "\n", req->src);
            }
        ...
        output->fatal(CALL_INFO, -1,
                      "Error: recv was called but the network is not configured yet, netID is still -1\n");
        return nullptr;
    }
}
...
void ShogunNIC::recvLinkEvent(SST::Event *ev) {
    ...
    if (nullptr != inEv) {
        output->verbose(CALL_INFO, 8, 0, "Recv link in handler: current pending request count: %5"
        PRIi32
        "\n", reqQ->count());

        if (reqQ->full()) {
            output->fatal(CALL_INFO, -1,
                          "Error - received a message but the NIC queues are full.\n");
            exit(-1);
        }
        ...
        if (nullptr != creditEv) {
            remote_input_slots++;
            output->verbose(CALL_INFO, 8, 0,
                            "Recv link credit event, remote_input_slots now set to: %5"
            PRIi32
            "\n", remote_input_slots);
        } else {
            ShogunInitEvent *initEv = dynamic_cast<ShogunInitEvent *>(ev);
            ...
            if (nullptr != initEv) {
                ...
                output->fatal(CALL_INFO, -1,
                              "Unknown event type received. Not sure how to process.\n");
            }
        }
...
void ShogunNIC::reconfigureNIC(ShogunInitEvent *initEv) {
    ...
    output->verbose(CALL_INFO, 4, 0, "Shogun NIC reconfig (%s) with %5"
    PRIi32
    " slots, xbar-total-ports: %5"
    PRIi32
    "\n",
    ...
    if (nullptr != output && netID > -1) {
        ...
        sprintf(outPrefix, "[t=@t][NIC%5"
        PRId64
        "][%25s][%5"
        PRId64
        "]: ", netID, getName().c_str(), netID);
