    ...
    namespace Shogun {
        ...
        public:
            ShogunStatisticsBundle(const int ports)
                ...
                output_packet_count = (Statistic <uint64_t> **) malloc(
                    sizeof(Statistic < uint64_t > *) * port_count);
                input_packet_count = (Statistic <uint64_t> **) malloc(
                    sizeof(Statistic < uint64_t > *) * port_count);
            ...
            void registerStatistics(SST::Component *comp) {
                ...
                for (int i = 0; i < port_count; ++i) {
                    sprintf(subIDName, "port%"
                    PRIi32, i);
                    output_packet_count[i] = comp->registerStatistic<uint64_t>(
                        "output_packet_count", subIDName);
                    input_packet_count[i] = comp->registerStatistic<uint64_t>("input_packet_count",
                                                                              subIDName);
                }
